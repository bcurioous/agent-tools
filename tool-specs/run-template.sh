#!/bin/bash
#===============================================================================
# Tool run.sh - Fixed Interface Harness (Pure Bash - No OpenCode Required)
#
# DO NOT MODIFY THIS FILE - This is the fixed interface for ALL tools.
# Each tool's specific logic goes in scripts/*.sh
#
# This is a standalone bash script - run it directly without OpenCode:
#
...
# Tool metadata (override in tool's SPEC.md)
TOOL_NAME="${TOOL_NAME:-$(basename "$(dirname "$0")")}"
...
load_limits() {
    if [ -f "$SCRIPT_DIR/LIMITS.md" ]; then
        # Parse LIMITS.md for runtime checks
...
check_limit() {
    ...
    # This is a stub - actual implementation in scripts/query.sh
